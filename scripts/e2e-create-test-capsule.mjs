/**
 * E2E helper: create ONE base-layer test capsule (no ER delegation).
 ...
 *
 * Exports createTestCapsule() for orchestrators; also runs as a CLI that prints the owner
 * pubkey on the last stdout line (machine-readable).
 *
...
/**
 * Create one base-only test capsule. Returns { owner, capsule, vault, createSig, vaultBalance }.
 */
export async function createTestCapsule({ inactivity = 10, totalAmountSol = '0.003' } = {}) {
...
// CLI entrypoint: node scripts/e2e-create-test-capsule.mjs [inactivity] [totalAmount]
...
if (isCli) {
  ...
  try {
    const r = await createTestCapsule({ inactivity, totalAmountSol })
